...
from petstore_api_test_project.api.delete_pet import delete_pet, delete_non_existent_pet
...
class TestPet:
    ...
    @allure.title('Update existing pet in the store')
    @allure.story('Update pet')
    @allure.tag('web')
    @allure.severity(Severity.CRITICAL)
    @allure.label('owner', 'SlinkovAA')
    def test_update_pet_name_in_store(self, api_url):
        new_pet = create_new_pet(api_url, pet_name='Harry')

        updated_pet = create_new_pet(api_url, pet_name='Potter')
    ...
    @allure.title('Delete non-existent pet in the store')
    @allure.story('Delete pet')
    @allure.tag('web')
    @allure.severity(Severity.CRITICAL)
    @allure.label('owner', 'SlinkovAA')
    def test_delete_non_existent_pet_from_store(self, api_url):
        new_pet = create_new_pet(api_url, pet_name='Lily')

        response, delete_pet_id = delete_non_existent_pet(api_url, new_pet)

        with allure.step('Проверка, что API возвращает код статуса 404 Not found.'):
            assert response.status_code == 404
